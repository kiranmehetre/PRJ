...
@Component({
  selector: 'app-std01-rhyming-words-worksheet11',
  templateUrl: './std01-rhyming-words-worksheet11.component.html',
  styleUrls: ['./std01-rhyming-words-worksheet11.component.scss']
})
export class Std01RhymingWordsWorksheet11Component implements OnInit {
...
$(document).ready(function(){
  ...
  $(inputs).css('cursor','pointer');
    ...
    $(".option").filter(function() {
      return $(this).find('img').attr('data-text').trim() === inputValue;
    }).removeClass('click');
    $(this).val("");
        });
  $('.option').click(function(){
            if(inputField !== undefined){
                inputValue=$(inputField).val();
      Text=$(this).find('img').attr('data-text').trim();
      HelpBox.push(Text);
  ...
  $('.button').click(function(){
    if($(this).hasClass('playAgain')){
      ...
      if(inputCount >= 6){
        $(inputs).removeClass('bd');
                    var Correct_Answers=[["stick","click"],["eat","beet"],["tent","bent"],["dice","rice"],["frame","flame"],["town","brown"]];
