...
@Component({
  selector: 'app-std03-proper-noun-worksheet4',
  templateUrl: './std03-proper-noun-worksheet4.component.html',
  styleUrls: ['./std03-proper-noun-worksheet4.component.scss']
})
export class Std03ProperNounWorksheet4Component implements OnInit {
    ...
    $(document).ready(function () {
      ...
      $('.button').click(function () {
        if ($(this).hasClass('playAgain')) {
          ...
          if (inputCount >= 5) {
            $(inputs).removeClass('bd');
            var Correct_Answers = [["Marriott Hotels"], ["Reebok"], ["Robin Hood"], ["Taco Bell"], ["Sun", "Earth"], ["Saturday", "Sunday"], ["Mozart"], ["The Ganges"]];
            $('.question').each(function (i) {
              ...
              if (i == 5) {
                ...
                for (var i = 0; i < Wrong_Answers.length; i++) {
                  ...
                  $(inputField_Array[i]).parent().siblings('.correct-answer').css('visibility', 'visible');
                }
              }
              else {
                $(inputRow).each(function (x) {
                  if ($.trim($(this).val().toString()) !== Correct_Answers[i][x]) {
                    $(this).css({ 'color': '#ed1a1a', 'border-color': '#fff' });
                    $(this).parent().siblings('.correct-answer').css('visibility', 'visible');
                    $(this).parent().siblings('.correct-answer').addClass('display');
                  }
                  else {
                    Correct_Answers_Count++;
                  }
