...
@Component({
  ...
  styleUrls: ['./std01-adjective-worksheet6.component.scss']
})
export class Std01AdjectiveWorksheet6Component implements OnInit {
...
$(document).ready(function(){
  ...
  $('.button').click(function(){
    if($(this).hasClass('playAgain')){
      ...
      if(inputCount >= 4){
        $(inputs).removeClass('bd');
        var Correct_Answers=["tasty","bright","juicy","pretty","colourful","brave","big","deep"];
        $(inputs).each(function(i){
          if($(this).val() !== Correct_Answers[i]){
            $(this).css({'color':'#ed1a1a','border-color':'#000'});
            $(this).parent().siblings('.correct-ans').css('display','block');
          }
          else{
