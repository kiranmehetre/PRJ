...
@Component({
  selector: 'app-std02-past-tense-worksheet4',
  templateUrl: './std02-past-tense-worksheet4.component.html',
  styleUrls: ['./std02-past-tense-worksheet4.component.scss']
})
export class Std02PastTenseWorksheet4Component implements OnInit {
    ...
    if($('.button').hasClass('playAgain'))
      ...
      else
      {
          ...
          if (inputCount >= 4) {
              ...
              var Correct_Answers = ["visited", "located", "built", "was", "shimmered", "sparkled", "studded", "said"];
              $(inputs).each(function (i) {
                  if ($.trim($(this).val().toString()) !== Correct_Answers[i]) {
                      $(this).css({ 'color': '#ed1a1a', 'border-color': '#000' });
                      $(this).siblings('.answer-container').css('visibility', 'visible');
                  }
                  else {
