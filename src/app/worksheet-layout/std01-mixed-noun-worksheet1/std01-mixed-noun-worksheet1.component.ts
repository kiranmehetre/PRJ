...
@Component({
  selector: 'app-std01-mixed-noun-worksheet1',
  templateUrl: './std01-mixed-noun-worksheet1.component.html',
  styleUrls: ['./std01-mixed-noun-worksheet1.component.scss']
})
export class Std01MixedNounWorksheet1Component implements OnInit {
      ...
      $('.button').click(function () {
        if ($('.button').hasClass('playAgain')) {
          ...
          if (inputCount >= 6) {
            ...
            var Correct_Answers = [];
            Correct_Answers[0] = ["Dr. Thomas", "Chef John", "policeman"];;
            Correct_Answers[1] = ["Kings Mall", "Wales Museum", "Ajax Stadium"];
            Correct_Answers[2] = ["camel", "giraffe", "crocodile"];
            Correct_Answers[3] = ["mobile", "flute", "truck"];
            ...
            for (var j = 0; j <= 3; j++) {
              ...
              // jQuery.grep(Correct_Answers[j], function (el) {
              //   if (jQuery.inArray(el, inputValues) == -1) Wrong_Answers.push(el);
              // });
            ...
            $('input,.option_Block').css('pointer-events', 'none');
          }
          else {
            setTimeout(function () {
              $('.result').css('display', 'flex');
